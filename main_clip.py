...
from clip_utils import cosine_lr, convert_models_to_fp32, refine_classname
...
def main():
    ...
    if args.dataset == 'cifar10':
        ...
        train_dataset, val_dataset = imagenet1000()
    ...
    if args.evaluate:
        ...
        if args.dataset == 'cifar':
            ...
            ood_val_loader = DataLoader(ood_test_set,
                                batch_size=args.batch_size, pin_memory=True,
                                num_workers=args.num_workers, shuffle=False)
            print('load corruption data: ', len(ood_val_loader))
            # acc1 = validate(val_loader, texts, model, prompter, criterion, args)
            acc1 = validate(ood_val_loader, texts, model, ssl_head, optimizer, lr_schedule_values, criterion, scaler, args)
        elif args.dataset == 'imagenet':        

            ood_test_set, orig_dataset = load_imagenetC(corruption, severity)  

            ood_val_loader = DataLoader(orig_dataset,
                                batch_size=args.batch_size, pin_memory=True,
                                num_workers=args.num_workers, shuffle=False)
            print('load corruption data: ', len(ood_val_loader))
            acc1 = validate(ood_val_loader, texts, model, ssl_head, optimizer, lr_schedule_values, criterion, scaler, args) 
...
def compute_reverse_transformation(model, model_ssl, optimizer, scheduler, criterion, scalar, X, epsilon, alpha, attack_iters, norm, aug_name, update_kernel, normalize, denormalize, args):
    ...
    if isRandom and isIter:
        ...
        for _ in range(attack_iters):
            ...
            if update_kernel == 'by_3' or update_kernel == 'by_5':
                kernel_param1 = init_kernel_param
                if args.dataset == 'cifar': new_x = trans_aug(aug_name, X, kernel_param1, factor_param_all)  + delta
                else: new_x = normalize(trans_aug(aug_name, denormalize(X), kernel_param1, factor_param_all))  + delta
            elif update_kernel == 'rand':
                if args.dataset == 'cifar': new_x = trans_aug(aug_name, X, kernel_param1, factor_param_all)  + delta
                else: new_x = normalize(trans_aug(aug_name, denormalize(X), kernel_param1, factor_param_all))  + delta
            elif update_kernel == 'comp':
                if args.dataset == 'cifar': 
                    new_x = trans_aug(aug_name, trans_aug(aug_name, X, kernel_param1, factor_param_all), kernel_param2, factor_param_all) + delta
                else: 
                    tmp_x = normalize(trans_aug(aug_name, denoramlize(X), kernel_param1, factor_param_all))
                    new_x = noramlize(trans_aug(aug_name, denormalize(tmp_x), kernel_param2, factor_param_all))+ delta
...
def validate(val_loader, texts, model, ssl_head, optimizer, scheduler, criterion, scalar, args):
    ...
    for i, (images, target, _) in enumerate(tqdm(val_loader)):
        ...
        print(images.shape)
        delta, kernel, param, before_loss, final_loss = compute_reverse_transformation(model, ssl_head, optimizer, scheduler, criterion, scalar, images,
        ...
        if args.update_kernel == 'comp':
            if args.dataset == 'cifar':
                ...
                new_x = trans_aug('sharpness', tmp_x, kernel[0], param_all) + delta
            else:
                tmp_x = normalize(trans_aug('sharpness', denormalize(images), kernel[1], param_all))
                new_x = normalize(trans_aug('sharpness', denormalize(tmp_x), kernel[0], param_all)) + delta
        
        else:
            if args.dataset == 'cifar':
                new_x = trans_aug('sharpness', images, kernel, param_all) + delta
            else:
                new_x = normalize(trans_aug('sharpness', denormalize(images), kernel, param_all)) + delta
        with torch.no_grad():
    ...
    with open(os.path.join('./output', args.output_fn), 'a') as f: 
        writer = csv.writer(f)
        writer.writerow(['l2', 'sharpness', 'corruption: '+ args.corruption, ' severity: '+ str(args.severity), 'batch-size: '+ str(args.batch_size), top1_org.avg, top1_adapt.avg])
