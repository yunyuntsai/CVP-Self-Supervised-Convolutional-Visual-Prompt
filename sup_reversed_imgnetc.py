...
def compute_reverse_attack(model, model_ssl, criterion, X, y, epsilon, alpha, attack_iters, norm):
    ...
    for _ in range(attack_iters):
        ...
        loss = -SslTrainer.compute_xentropy_loss(new_x, criterion, model, y, no_grad=False)
...
def test_acc_reverse_vector(model, model_ssl, test_batches_orig, test_batches, criterion, attack_iters, args, normalize, denormalize):
    ...
    for counter in range(n_batches):
        ...
        if args.aug_name is None:
            delta = compute_reverse_attack(model, model_ssl, criterion, x, y,
...
class SslTrainer:
    ...
    @staticmethod
    def compute_xentropy_loss(x, criterion, model, label, no_grad=True):
        if no_grad:
            with torch.no_grad():
                logits, _ = model(x)
        else:
