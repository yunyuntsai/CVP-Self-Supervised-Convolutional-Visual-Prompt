...
def test_acc(model, test_batches, normalize, imagenet_a_mask):
    acc = 0
    test_size  = 0
    ...
    if args.eval:
        ...
        for rm in reverse_method:
            ...
            for attack_iter in attack_iters:
                # acc1, acc2, reverse_loss_list, corr_loss_list = test_acc_reverse_vector_aug(model, dist_head, test_batches_ood, rot_transform, criterion, 0,  args.aug_name, normalize, denormalize, imagenet_a_mask)
                if args.allow_adapt:
